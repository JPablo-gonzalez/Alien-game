...
let displayLives state =
    ...
    Utils.displayMessage (state.Width/2-10) 0 ConsoleColor.Yellow vidas
...
let mostrarJuego (savedState: State option) =
    let initialState = 
        match savedState with
        | Some s -> { s with ProgramState = Running } 
        | None -> initSate()
    ...
    |> fun state ->
        if state.AlienLives <= 0 then
            GamesStatus.GameOver, state 
        else
            GamesStatus.Pause, state